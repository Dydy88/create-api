 ...
 * JSON a envoyé avec la méthode POST. Pour effextuer la création
 {
    "nom":"Produit1",
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($donnees->nom) && !empty($donnees->description) && !empty($donnees->prix) && !empty($donnees->categories_id)) {
        ...
        if ($produit->creer()) {
            // Ici la création a fonctionné, On envoie un code 201
        ...
        http_response_code(503);
        echo json_encode(["message" => "Tous les paramètres n'ont pas été passé."]);
    }
} else {
